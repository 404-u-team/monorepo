...
func (h *projectRequestHandler) AcceptProjectRequest(c *gin.Context) {
	...
	// одобрить заявку
	projectRequest, err := h.projectRequestService.UpdateProjectRequestStatus(projectRequestID, userID, "accepted")
	if err != nil {
...
func (h *projectRequestHandler) RejectProjectRequest(c *gin.Context) {
	...
	// одобрить заявку
	projectRequest, err := h.projectRequestService.UpdateProjectRequestStatus(projectRequestID, userID, "rejected")
	if err != nil {
		if errors.Is(err, services.ErrUserNotLeader) || errors.Is(err, services.ErrProjectRequestDontBelongToUser) {
			c.JSON(http.StatusForbidden, gin.H{"error": err.Error()})
			return
