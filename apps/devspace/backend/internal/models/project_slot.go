...
type ProjectSlot struct {
	...
	ProjectId       uuid.UUID  `gorm:"column:project_id;type:uuid; not null;uniqueIndex:idx_project_user"`
	SkillCategoryId uuid.UUID  `gorm:"column:skill_category_id;type:uuid; not null"`
	UserId          *uuid.UUID `gorm:"column:user_id;type:uuid;uniqueIndex:idx_project_user"`
