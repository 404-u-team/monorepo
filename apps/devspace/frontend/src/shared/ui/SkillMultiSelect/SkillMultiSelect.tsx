...
export function SkillMultiSelect({
    ...
    useEffect(() => {
        ...
        void loadOptions(debouncedQuery).then((result) => {
            if (!cancelled) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only reload on debouncedQuery/isOpen/selectedIds change
    }, [debouncedQuery, isOpen, value.length]);

    // Close on outside click
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent): void => {
            if (containerReference.current !== null && !containerReference.current.contains(event.target as Node)) {
                setIsOpen(false);
            }
        };
    ...
    const handleSelect = useCallback((option: SkillMultiSelectOption): void => {
        if (isAtMax) return;
    ...
    const handleRemove = useCallback((skillId: string): void => {
    ...
    return (
        ...
        >
            ...
            >
                ...
                {isAtMax && (
                    <span className={styles.maxHint}>Макс. {max}</span>
                )}
            </div>
